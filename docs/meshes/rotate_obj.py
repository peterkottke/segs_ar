    ...
    with open( filename, 'r' ) as objf:
        ...
        for line in objf:
            ...
            if not toks:
                continue
            if toks[0] == 'v':
                obj.vertices.append( [ float(v) for v in [toks[1], toks[3], toks[2]]] )
                # obj.vertices.append([float(toks[1]))
                # obj.vertices.append(float(toks[3]))
                # obj.vertices.append(float(toks[2]))
            elif toks[0] == 'vn':
...
if __name__ == "__main__":
    for i in range(1,9):
        save_obj(load_obj(f"S{i}_RH.obj"), f"S{i}_LH.obj")